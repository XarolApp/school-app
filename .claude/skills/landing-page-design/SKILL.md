...
> **Local addition (not in the upstream source) — project conflict check.**
> Before applying any Part B rule (fonts, icons, spacing/radius scale, colors,
> motion curves) to a project that already has its own established design
> system — a `DESIGN.md`, a `tokens.js`/design-tokens file, a documented
> component library, or an explicit instruction in that project's own
> CLAUDE.md/README — **stop and ask the user** which system should win for
> that specific value, rather than silently applying either this skill's
> default or the project's existing one. Name the specific conflicting values
> when asking (e.g. "this skill specifies Geist + Tailwind's spacing scale;
> this project's `tokens.js` specifies Fraunces/Public Sans and its own
> spacing tokens — which should the landing page use?"). Part A (strategy,
> structure, copywriting) has no such conflict risk and can be applied
> normally — this check is scoped to Part B's concrete visual values only.
