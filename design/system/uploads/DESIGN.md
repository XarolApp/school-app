...
1. **A 131KB sourced research document** (`docs/sources/design_direction_research.md`)
...
**For whoever builds screens from this file (including Claude Design): query the
Mobbin MCP directly before generating anything.** This file describes a direction in
prose and tokens; it is not a substitute for looking at real screens. If Mobbin is
available, search it for the direction named below (warm neutral grounds, one
desaturated accent, generous radius, real photography) before drawing a single pixel.
The ten screens cited here are a starting point, not the full research. When evaluating
a reference screen, check that it's not fundamentally misaligned with the warm-but-credible
direction — exclude sites serving opposite purposes (cold developer tools, pure entertainment,
...
engine** (see `docs/sources/design_direction_interview.md`, "still open" — the
...
(`docs/sources/landing_animation_research_prompt.md`), findings in
`docs/sources/landing_animation_research.md` (2026-08-28). Unlike the Mobbin
...
**The actual pattern, corrected from an earlier draft of this section:**
this is **ambient, autoplay motion that runs on page load with no user
action** — ONE small looping element, not scroll-triggered entrances and not
hover/press feedback (both of those are ordinary interaction motion, covered
in Do's/Don'ts below, and are a different thing from what's being specified
here). The landing page is the one screen in the product where this kind of
warmth-building motion belongs at all — see the Do's/Don'ts note on why the
quiz and results screens are the opposite case.

**What ships:**
- **Exactly one ambient loop**, small and contained — a corner illustration,
  an icon, or a subtle background gradient/shape drift. Never full-screen,
  never the literal subject of the hero (the headline and CTA stay the focal
  point). Research finding: scale and containment are what separate "polish"
  from "trying too hard" — a large central animated character reads as
  juvenile for a decision-context audience, a parent evaluating whether to
  pay specifically.
- **Slow and unvarying** — multi-second cycle, eased, no sudden bursts or
  escalating gestures. Research finding: speed is the strongest signal here —
  slow smooth motion (the Linear/Stripe gradient-drift register) reads
  premium/calm; fast or bouncy motion reads young, which is exactly the
  territory this product's parent-facing side cannot afford.
- **Technique, in priority order for this product's mobile-first, budget
  constraints:**
  1. CSS keyframe animation (gradient/shape drift) — cheapest, GPU-cheap via
     `transform`/`opacity` only, zero JS cost after paint. Default choice.
  2. A single small Lottie or Rive loop (the actual Duolingo-style
     character/illustration idle-loop technique) — only if a designed
...
- No mascot, no bouncing character, no confetti — confetti stays reserved
  for the in-product student reveal screen (see onboarding); spending it
  pre-signup burns the one payoff it has. Research names confetti/particle
  effects as the single highest-risk pattern for reading as childish/gamified
  on a first-impression, trust-building screen.
- No counting-up numbers or fake "calculating" states — a real stat can be
  present, but does not get a live-computation-implying counter effect (see
  Colors → tertiary rule and the no-invented-numbers rule elsewhere in this
  file).
- No second competing ambient element — one loop only. Multiple simultaneous
  ambient animations dilute the "made, not templated" signal into visual
  noise, and compete with the CTA for attention.

**Always:** the ambient loop is fully disabled under
...
house style here: the research confirms `prefers-reduced-motion` exists
because motion (background/ambient motion specifically) can trigger real
vestibular symptoms, not just a stylistic preference. Matches the pattern
already used throughout onboarding (`usePrefersReducedMotion`), extended to
the landing page.
...
1. Use **Claude Design's dedicated animation tool ONLY** — pass this entire "Motion — landing page" section + `docs/sources/landing_animation_research.md` to that tool
...
- **Do** limit motion to confirming an action or explaining a spatial change, using
  short, purposeful durations (~150ms state changes, ~250ms transitions). **Don't**
  animate section entrances on scroll *inside the quiz or at the results screen*,
