...
Historical migration notes and completed design-session logs are kept in git history, the archive, or `UNFORGET.md`; they do not belong in this active handoff. Keep this file focused on current architecture, locked decisions, operational rules, and known traps.

Update both `AGENTS.md` and `CLAUDE.md` together when a project fact changes. Their shared content should stay equivalent; only agent-specific paths and invocation notes may differ.

## Keeping This File Useful

This file is read at the start of every session. Update it when a feature moves from not built to built, a convention changes, or a product decision becomes final. All deferred work, pending decisions, TODOs, audit findings, and paused plans go in [`UNFORGET.md`](UNFORGET.md).
