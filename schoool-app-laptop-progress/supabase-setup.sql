...
create table if not exists public.users (
  ...
  subscription_status text not null default 'trialing'
    check (subscription_status in ('trialing', 'active', 'past_due', 'canceled', 'expired', 'developer')),
...
-- Supabase Auth creates an account, so the 7 days is not negotiable.
-- ----------------------------------------------------------------------------
  ...
  select exists (
    ...
    where u.id = uid
      and (
        ...
        or u.subscription_status in ('active', 'developer')
      )
  );
$$;
