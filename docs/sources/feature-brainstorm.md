...
| Full share-with-parent link (saved schools + questionnaire results + notes, not just the shortlist) | 🔥 | Broader than the row above — one link gives the parent read-only access to everything the student has done in the app (saved schools, match scores, questionnaire answers/results, any notes), not only the final shortlist. This is what makes the parent-branch paywall credible: instead of a cold pitch, the parent opens the link and sees their own kid's actual work already done, which is a much stronger reason to pay than a generic sales screen. Needs a read-only, no-signup-required view (a share token, not a full account) so the parent doesn't have to make an account just to look. **Ships together with the reverse direction, not separately:** a parent on the parent branch sending the questionnaire to their child to fill in on the child's own device is the same infrastructure — share token + cross-device session sync — pointed the other way. Building one without the other means writing that plumbing twice. Until it exists, the parent branch shows an inert, explicitly-unbuilt "Poslat odkaz dítěti" control on the first quiz question plus a same-device "hand them the phone" nudge (`QuizQuestion.jsx`) |
