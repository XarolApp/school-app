...
// The model only writes one Czech sentence per school from evidence the scorer
// already produced, so a cheap tier is the right choice — Gemini 2.5 Flash Lite
// is ~20x cheaper than Sonnet for this and reliable at strict JSON. Its Czech
// has not had a human quality pass yet (see UNFORGET.md). Override with
// OPENROUTER_MODEL to switch without a code change.
...
async function requestMatches({ answers, schools, apiKey, model, referer }) {
  ...
  const shortlist = scoreSchools(answers, schools).slice(0, REASON_COUNT);
