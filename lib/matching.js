...
function isGymnasium(school) {
  return shareOf(school._programs, GYMNASIUM_KEYWORDS) >= TYPE_MAJORITY;
}
function isLyceum(school) {
  return shareOf(school._programs, LYCEUM_KEYWORDS) >= TYPE_MAJORITY;
}
function isTrade(school) {
  return shareOf(school._programs, TRADE_KEYWORDS) >= TYPE_MAJORITY;
}
...
function scoreSchools(answers, schools) {
  return schools
    .map((school) => {
      ...
      const raw = applicable ? (total / applicable) * 100 : 0;
