...
// JS \b only understands ASCII word characters, so it misses ředitel
// at the start of a word. Accept sentence-initial capitals as well. Keep surnames case-sensitive.
const NAMED_PERSON = /(?<![\p{L}\p{N}_])(?:(?:[Pp]an|[Pp]aní)(?:\s+(?:[Uu]čitel(?:ka)?|[Pp]rof\.?|[Pp]rofesor(?:ka)?|[Řř]editel(?:ka)?))?|[Uu]čitel(?:ka)?|[Pp]rof\.?|[Pp]rofesor(?:ka)?|[Řř]editel(?:ka)?)\s+\p{Lu}\p{Ll}+/u;
